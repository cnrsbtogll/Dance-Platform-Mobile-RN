...
export const NewChatScreen: React.FC = () => {
  ...
  // Get instructors list (for students)
  const instructors = useMemo(() => {
    const allInstructors = MockDataService.getUsersByRole('instructor');
    if (!searchQuery.trim()) {
      return allInstructors.filter(instructor => instructor.id !== user?.id);
    }
  ...
  const handleSelectUser = (instructorId: string) => {
      ...
      <View style={[styles.searchContainer, { backgroundColor: palette.background, borderBottomColor: palette.border }]}>
        <View style={[styles.searchBar, { backgroundColor: palette.card, borderColor: palette.border }]}>
          ...
          <TextInput
            ...
            placeholder={t('chat.searchInstructors')}
        ...
        {instructors.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyStateTitle, { color: palette.text.primary }]}>
              {searchQuery ? t('chat.noResults') : t('chat.noInstructors')}
            </Text>
            <Text style={[styles.emptyStateText, { color: palette.text.secondary }]}>
              {searchQuery ? t('chat.noResultsDescription') : t('chat.noInstructorsDescription')}
            </Text>
          </View>
        ) : (
          <View style={styles.usersList}>
            {instructors.map((instructor) => {
              const instructorLessons = MockDataService.getLessonsByInstructor(instructor.id);
              const primaryCategory = instructorLessons[0]?.category || '';
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  onPress={() => handleSelectUser(instructor.id)}
                >
                  <View style={styles.userContent}>
                    ...
                    <View style={styles.userInfo}>
                      ...
                      <Text style={[styles.userSubtitle, { color: palette.text.secondary }]} numberOfLines={1}>
                        {primaryCategory 
                          ? `${primaryCategory} ${t('chat.instructorSuffix')}`
                          : t('chat.instructor')
                        }
                      </Text>
                      {instructor.bio && (
